    ...
    if ($export_format === 'pdf') {
        ...
        switch($user_role) {
            case 'super_admin':
                ...
                $report_description = 'Custom report covering all wards and villages in the system.';
                break;
            case 'admin':
                $report_heading = 'Ward Registration Report - Ward Administration';
                $report_description = 'Custom report for ward-level administration.';
                break;
            case 'weo':
                $report_heading = 'Ward Registration Report - Ward Executive Officer';
                $report_description = 'Custom report for ward management.';
                break;
            case 'veo':
                $report_heading = 'Ward Registration Report - Village Executive Officer';
                $report_description = 'Custom report for village management.';
                break;
            default:
                $report_heading = 'Ward Registration Report';
                $report_description = 'Custom report for residence registrations.';
        }
        ...
        $pdf->SetKeywords('Ward, Registration, Custom, Report');
        ...
        $pdf->Cell(0, 8, 'Report Type: ' . ucfirst($report_type), 0, 1, 'L');
        $pdf->Cell(0, 8, 'Date Range: ' . $date_from . ' to ' . $date_to, 0, 1, 'L');
        $pdf->Ln(10);
        
        // Data table
        if (!empty($report_data)) {
            $pdf->SetFont('helvetica', 'B', 14);
            $pdf->Cell(0, 10, 'Custom Report Data', 0, 1, 'L');
            $pdf->Ln(5);
            ...
            foreach ($headers as $header) {
                $header_widths[] = 30; // Default width
            }
            
            // Print headers
            foreach ($headers as $i => $header) {
                $pdf->Cell($header_widths[$i], 8, ucwords(str_replace('_', ' ', $header)), 1, 0, 'C', 1);
            }
            $pdf->Ln();
            ...
            foreach ($report_data as $row) {
                foreach ($row as $i => $value) {
                    $pdf->Cell($header_widths[$i], 8, $value, 1, 0, 'L', 0);
                }
                $pdf->Ln();
            }
        } else {
        ...
        // Write report header
        fputcsv($output, array('Ward Registration System - Custom Report'));
        ...
        fputcsv($output, array('Report Type: ' . ucfirst($report_type)));
        fputcsv($output, array('Date Range: ' . $date_from . ' to ' . $date_to));
        fputcsv($output, array(''));
        
        // CSV headers
        fputcsv($output, array('CUSTOM REPORT DATA'));
        if (!empty($report_data)) {
            fputcsv($output, array_keys($report_data[0]));
            
            // Data rows
            foreach ($report_data as $data_row) {
                fputcsv($output, array_values($data_row));
            }
        }
...
function exportToExcel() {
    // Server-side Excel export (using CSV format)
